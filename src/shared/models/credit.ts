...
/**
 * Get remaining credits for multiple users (single query).
 * 
 * @param userIds - Array of user IDs to fetch credits for
 * @returns Record mapping user ID to their remaining credits count
 * 
 * Note: Returns empty object {} if query fails, allowing graceful degradation
 * rather than crashing the calling page. Errors are logged for debugging.
 */
export async function getRemainingCreditsByUserIds(
  userIds: string[]
  ...
  try {
    ...
    const rows = await db()
      .select({ userId: credit.userId, total: sum(credit.remainingCredits) })
      .from(credit)
      .where(
        and(
    ...
    for (const row of rows) {
      ...
      const normalizedTotal =
        typeof total === 'bigint'
          ...
          : typeof total === 'number'
            ? total
            : parseInt(total as any); // drizzle may return string

      creditsMap[row.userId] = Number.isNaN(normalizedTotal)
        ? 0
        : normalizedTotal;
    }
    ...
    console.error('Failed to fetch remaining credits by user IDs:', {
      error,
    ...
    // Return empty map to allow page to load gracefully
    return {};
  }
}
