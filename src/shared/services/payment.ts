...
export async function handleCheckoutSuccess({
  ...
  if (session.paymentStatus === PaymentStatus.SUCCESS) {
    ...
    await trackServerEvent({
      eventName: 'purchase',
      dedupeKey: `purchase:${orderNo}`,
      source: 'server',
      userId: order.userId,
      orderNo,
      properties: {
        transaction_id: orderNo,
        payment_provider: order.paymentProvider || session.provider || '',
        ...
        value:
          typeof session.paymentInfo?.paymentAmount === 'number'
            ? Number((session.paymentInfo.paymentAmount / 100).toFixed(2))
            : typeof order.paymentAmount === 'number'
              ? Number((order.paymentAmount / 100).toFixed(2))
              : typeof order.amount === 'number'
                ? Number((order.amount / 100).toFixed(2))
                : undefined,
        subscription_id: session.subscriptionId || order.subscriptionId || '',
      },
    });
  } else if (
          ...
          typeof session.paymentInfo?.paymentAmount === 'number'
            ...
            : typeof order.paymentAmount === 'number'
              ...
              : typeof order.amount === 'number'
                ? Number((order.amount / 100).toFixed(2))
                : undefined,
        subscription_id: session.subscriptionId || subscription.subscriptionId,
      },
    });
  } else {
    throw new Error('unknown payment status');
