...
export function VerifyEmailPage({
  ...
  const { configs, hasFetchedConfigs } = useAppContext();
  ...
  const completeVerificationAndNavigate = useCallback(
    (sessionUser?: { id?: string | null; email?: string | null }) => {
      if (typeof window !== 'undefined') {
        const pendingEmailSignUp = getPendingEmailSignUp();
        const sessionEmail = String(sessionUser?.email || email || '')
          .trim()
          .toLowerCase();

        if (pendingEmailSignUp?.email && sessionEmail) {
          if (pendingEmailSignUp.email === sessionEmail) {
            const trackedKey = `gtm_verified_email_sign_up_tracked_${
              sessionUser?.id || sessionEmail
            }`;
            if (window.localStorage.getItem(trackedKey) !== '1') {
              ...
              trackConfiguredGtmSignUp({
                method: 'email',
                configs,
                onComplete: hardNavigateToNextUrl,
              });
              return;
            }
          }
    ...
    [configs, email, hardNavigateToNextUrl, hasFetchedConfigs]
  );
  ...
  const checkSessionAndRedirect = useCallback(async () => {
    ...
    try {
      ...
      if (data?.user) {
        completeVerificationAndNavigate(data.user);
      }
    } catch {
      // ignore
    }
  }, [completeVerificationAndNavigate]);
  ...
  useEffect(() => {
    ...
    if (!isPending && session?.user) {
      completeVerificationAndNavigate(session.user);
    }
  }, [completeVerificationAndNavigate, isPending, session?.user]);
