...
export function MusicGenerator({ className, srOnlyTitle }: SongGeneratorProps) {
  ...
  const { user, isCheckSign, setIsShowSignModal, fetchUserCredits, configs } =
    useAppContext();
  ...
  const handleGenerate = async () => {
    ...
    const analyticsSnapshot: GenerateAnalyticsSnapshot = {
      provider,
      model,
      mode: customMode ? 'custom' : 'prompt',
      costCredits,
      promptLength: customMode ? lyrics.length : prompt.length,
    };
