...
const HeaderSignUser = dynamic(
  () => import('@/shared/blocks/sign/sign-user').then((mod) => mod.SignUser),
  {
    ...
    loading: () => (
      <div
        aria-hidden="true"
        className="h-11 w-36 rounded-full border border-black/6 bg-white/80 shadow-sm"
      />
    ),
  }
);
              ...
              return (
                <NavigationMenuLink key={idx} asChild>
                  <Link
                    ...
                    className={cn(
                      'flex flex-row items-center gap-2 rounded-full px-4 py-1.5 text-sm transition-colors',
  ...
  return (
    <>
      <header
        ...
        className={cn(
          'fixed inset-x-0 top-0 z-50 has-data-[state=open]:h-screen has-data-[state=open]:backdrop-blur',
          isHeroOverlay
            ? 'text-white'
            : 'has-data-[state=open]:bg-background/50'
        )}
      >
        <div
          className={cn(
        ...
        >
          <div
            className={cn(
              ...
              isHeroOverlay &&
                '[&_[data-slot=button]]:border-white/10 [&_[data-slot=button]]:bg-transparent [&_button]:text-white [&_button]:hover:bg-white/10'
            )}
          >
                ...
                <div className="flex w-full flex-row items-center gap-4 sm:flex-row sm:gap-6 sm:space-y-0 md:w-fit">
                  ...
                  {header.show_sign ? (
                    <HeaderSignUser
                      ...
                      isScrolled={!isHeroOverlay && isScrolled}
                    />
                  ) : null}
                </div>
              </div>
