      ...
      <div className="mx-auto mb-12 text-center">
        ...
        {blog.buttons && blog.buttons.length > 0 && (
          <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
            {blog.buttons.map((button, index) => (
              <Button
                key={`${button.url || button.title || 'blog-button'}-${index}`}
                asChild
                variant={button.variant || 'outline'}
                size={button.size || 'default'}
              >
                <Link href={button.url || '/blog'} target={button.target}>
                  {button.icon && <SmartIcon name={button.icon as string} size={16} />}
                  {button.title}
                </Link>
              </Button>
            ))}
