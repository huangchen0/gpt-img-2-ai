...
export default async function AdminUsersPage({
  ...
  // Fetch credits with error handling for graceful degradation
  let creditsMap: Record<string, number> = {};
  try {
    creditsMap = await getRemainingCreditsByUserIds(userIds);
  } catch (error) {
    console.error('Failed to load user credits on admin page:', error);
  ...
  const table: Table = {
    columns: [
      ...
      },
      {
        ...
        callback: async (item: User) => {
          const credits = creditsMap[item.id] ?? 0;
      ...
      { name: 'referrerDomain', title: t('fields.referrer_domain') },
      {
        name: 'actions',
