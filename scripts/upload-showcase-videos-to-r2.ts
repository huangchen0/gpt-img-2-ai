...
async function uploadVideosToR2() {
  try {
    ...
    if (videoFiles.length === 0) {
      console.error('✗ No video files found in public/showcases/videos/');
      process.exit(1);
    }
    ...
    for (const filename of videoFiles) {
      ...
      try {
        ...
        if (result.success && result.url) {
          ...
          // Find matching showcase record by filename
          ...
          if (showcases.length > 0) {
            // Update showcase record with R2 URL
            ...
            console.log(`  ⚠ No matching showcase found for ${filename}`);
          }
    ...
    if (failed > 0) {
      ...
      console.log('\nShowcase videos are now served from R2 CDN.');
